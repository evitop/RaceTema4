...
```{r}
cran3 %>% summarise(avg_bytes = median(size))
```
# Agrupamientos
...
```{r}
cran3 %>% group_by(by_package = package)
...
```{r}
cran3 %>% group_by(package) %>% summarise(avg_size = median(size))
...
pack_sum <- cran %>% group_by(package) %>% summarise(count = n(), unique = n_distinct(ip_id), countries = n_distinct(country), avg_bytes = mean(size))
